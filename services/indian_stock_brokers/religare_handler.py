...
async def handle_religare_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://api.religareonline.com/{clean_path}"

    log = AuditLog(broker="religare", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def religare_health():
    return {"broker": "religare", "status": "handler_active"}
