...
async def handle_kotak_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://gw-napi.kotaksecurities.com/{clean_path}"

    log = AuditLog(broker="kotak", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def kotak_health():
    return {"broker": "kotak", "status": "handler_active"}
