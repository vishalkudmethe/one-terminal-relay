...
async def handle_nuvama_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://api.nuvamawealth.com/{clean_path}"

    log = AuditLog(broker="nuvama", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def nuvama_health():
    return {"broker": "nuvama", "status": "handler_active"}
