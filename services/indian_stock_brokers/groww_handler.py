...
async def handle_groww_request(path: str, request: Request, db, xff, uuid):
    """Core Groww Request Handler"""
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://growwapi.groww.in/{clean_path}"

    log = AuditLog(broker="groww", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
async def groww_health():
    return {"broker": "groww", "status": "handler_active"}
