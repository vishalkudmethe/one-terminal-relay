...
async def handle_icici_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://api.icicidirect.com/breezeapi/api/v1/{clean_path}"

    log = AuditLog(broker="icici", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def icici_health():
    return {"broker": "icici", "status": "handler_active"}
