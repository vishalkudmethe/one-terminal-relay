...
async def handle_hdfc_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://api.hdfcsec.com/{clean_path}"

    log = AuditLog(broker="hdfc", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def hdfc_health():
    return {"broker": "hdfc", "status": "handler_active"}
