...
async def handle_profitmart_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://api.profitmart.in/{clean_path}"

    log = AuditLog(broker="profitmart", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def profitmart_health():
    return {"broker": "profitmart", "status": "handler_active"}
