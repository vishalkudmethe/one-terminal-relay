...
async def handle_motilal_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://openapi.motilaloswal.com/RestApi/{clean_path}"

    log = AuditLog(broker="motilal", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def motilal_health():
    return {"broker": "motilal", "status": "handler_active"}
