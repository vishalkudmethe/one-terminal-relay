...
async def handle_prabhudas_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://api.plindia.com/{clean_path}"

    log = AuditLog(broker="prabhudas", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def prabhudas_health():
    return {"broker": "prabhudas", "status": "handler_active"}
