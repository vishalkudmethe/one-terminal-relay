...
async def handle_zerodha_request(path: str, request: Request, db, xff, uuid):
    """Core Zerodha/Kite Connect Request Handler"""
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://api.kite.trade/{clean_path}"

    log = AuditLog(broker="zerodha", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def zerodha_health():
    return {"broker": "zerodha", "status": "handler_active"}
