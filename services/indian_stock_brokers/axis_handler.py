...
async def handle_axis_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://rapidapi.axisdirect.in/{clean_path}"

    log = AuditLog(broker="axis", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def axis_health():
    return {"broker": "axis", "status": "handler_active"}
