...
async def handle_iifl_request(path: str, request: Request, db, xff, uuid):
    clean_path = urllib.parse.unquote(path).strip('/')
    url = f"https://api.iiflsecurities.com/{clean_path}"

    log = AuditLog(broker="iifl", endpoint=path, method=request.method, client_ip=xff, device_uuid=uuid)
...
@router.get("/health")
async def iifl_health():
    return {"broker": "iifl", "status": "handler_active"}
